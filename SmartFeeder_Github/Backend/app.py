...
GPIO.setup(servoVoeder,GPIO.OUT)
GPIO.setup(servoBakje,GPIO.OUT)

###### Objecten van klasses maken ######
shiftObj = shiftregister(DS, OE, STCP, SHCP, MR)
lcdObj = lcd(E, rs, shiftObj)
hx = HX711(HXdata, HXclk)
pwmVoeder=GPIO.PWM(servoVoeder, 50)
pwmBakje=GPIO.PWM(servoBakje, 50)
...
def servoVoederBeweging():
    
    SetAngle(servoVoeder,pwmVoeder,8)
    SetAngle(servoVoeder,pwmVoeder,1)
    SetAngle(servoVoeder,pwmVoeder,8)
    print("Geloopt")
    time.sleep(1)
...
def schrijven_op_lcd():
    ...
    adressen_string = adressen_string[15:28]
    adressen_string = "IP:"+adressen_string
    print(adressen_string)
    # ip weergeven op lcd
...
# lcd_IP = threading.Thread(target=schrijven_op_lcd)
# lcd_IP.start()
time.sleep(3)
lcd_setup()
schrijven_op_lcd()
